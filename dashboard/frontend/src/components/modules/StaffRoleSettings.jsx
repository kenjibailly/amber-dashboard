...
export default function StaffRoleSettings({ guildId, user }) {
  ...
  const fetchSettings = async () => {
    try {
      const response = await axios.get(`/guilds/${guildId}/modules/staffrole`, {
      ...
      console.error("Failed to fetch staff role settings:", err);
    }
  };
  ...
  const handleSave = async (e) => {
    ...
    try {
      await axios.put(
        `/guilds/${guildId}/modules/staffrole/settings/${user.id}/@${user.username}`,
        {
          settings,
        },
        { withCredentials: true },
    ...
    <form onSubmit={handleSave} className={styles.settingsForm}>
      <div className={styles.formGroup}>
        ...
        <small className={styles.hint}>
          This role will be used as the staff role for different functions where
          the staff is being used
        </small>
      </div>
