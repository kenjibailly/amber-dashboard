...
export default function DailyBrawlerPollSettings({ guildId, user }) {
  ...
  const fetchSettings = async () => {
    try {
      const response = await axios.get(
        `/guilds/${guildId}/modules/dailybrawlerpoll`,
        {
  ...
  const handleSave = async (e) => {
    ...
    try {
      await axios.put(
        `/guilds/${guildId}/modules/dailybrawlerpoll/settings/${user.id}/@${user.username}`,
        {
          settings,
        },
        { withCredentials: true },
    ...
    <form onSubmit={handleSave} className={styles.settingsForm}>
      <div className={styles.formGroup}>
        ...
        <small className={styles.hint}>
          Select the channel where daily brawler poll will be sent
        </small>
      </div>
